...
// I will just use a split or indexOf something very specific like:
// 'fa-paper-plane text-sm"></i>\n                  </button>\n                </div>\n              </div>'
const anchor = 'fa-paper-plane text-sm"></i>\n                  </button>\n                </div>\n              </div>';
const idx = content.lastIndexOf(anchor);
if (idx === -1) {
    console.log("Anchor not found!");
} else {
    const start = content.slice(0, idx + anchor.length);
    const end = `
            </div>
          )}
        </div>
...
`;
    fs.writeFileSync('components/GameView.tsx', start + end);
